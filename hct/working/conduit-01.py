...
def fit():
    ...
    m_constructors = {
        'xgb': partial(xgb.XGBRegressor, enable_categorical=True),
        'lgb': partial(lgb.LGBMRegressor, verbose=-1),
        'cb': partial(cb.CatBoostRegressor, silent=True),
    }
...
def cv_score():
    ...
    for r in range(1, len(y_pred_oofs)+1):
        for m_combo in combinations(y_pred_oofs.items(), r):
            m_names, ranked_risk_score = zip(*m_combo)
            m_names = '-'.join(m_names)
            score = calc_score(train, sum(ranked_risk_score))
...
def predict():
    ...
    with open(fn, newline='') as f:
        for row in csv.DictReader(f):
            cv_score = float(row['score'])
            m_names = row['models'].split('-')

    print(f'using ensemble with best CV score: {cv_score:.4f} {m_names}')
    Xs = get_Xs('test')
    m_name_Xs = []
