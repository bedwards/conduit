...
def get_X(train_or_test, name):
    ...
    X = pd.concat(
        [train.drop(columns=["Survived"] if "Survived" in train else []), test]
    )
    ...
    for col in X.columns:
        if col in Xi.columns or col in Xf.columns:
...
def cv_score():
    ...
    scores = sorted(scores)  # Sort ascending to match original script
